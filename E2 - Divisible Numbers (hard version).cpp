...
int main(){
    ...
    while(t--){
        ...
        ll bare_min= a*b, max_lim= c*d;
        bool pos=0;

        ll ans1=-1, ans2=-1;
        for(ll i=1; bare_min*i<= max_lim and !pos; i++ ){

            ll temp= i*bare_min;

            for(ll j=2;!pos and j*j<=temp; j++){
                if(temp%j==0){
                    ll fm=j, sm= temp/j;

                    if(a<fm and fm<=c and b<sm and sm<=d){
                        ans1=fm; ans2=sm;
                        pos=1;
                    }
                    else if(a<sm and sm<=c){
                        if(b<fm and fm<=d){
                            ans1=sm; ans2=fm;
                            pos=1;
                        }
